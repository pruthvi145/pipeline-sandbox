...
// Scope matches migrate-supabase's own migration surface plus the ledger table
// that keeps a future restore's migration history consistent with the schema
// it restores. Deliberately excludes auth/storage/extensions - those are
// Supabase-managed and a dump/restore of them would fight the platform.
const SNAPSHOT_SCHEMAS = ["public", "stripe", "audit", "supabase_migrations"];
...
async function main() {
  ...
  const existing = (listing.Contents || []).map((obj) => ({
    key: obj.Key,
    lastModified: obj.LastModified,
  }));
  const toDelete = filesToPrune(existing);
